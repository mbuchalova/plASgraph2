...
class plasgraph(keras.Model):
    def __init__(
        ...
        if self['tie_gnn_layers']:
            ...
            # norm_list = [] # temporary list of normalization layers
            ...
            for layer_idx in range(self['n_gnn_layers']):
                ...
                # norm_name = f"gnn_norm_{layer_idx}"
                ...
                # norm_list.append(getattr(self, norm_name))
            ...
            # self._gnn_norm = NoDependency(norm_list)
    ...
    def call(self, inputs):
        ...
        for gnn_layer in range(self['n_gnn_layers']):
            ...
            # if self._gnn_norm is not None:
            #     x = self._gnn_norm[gnn_layer](x)
            x = self._gnn_dropout_pre_fully_connected[gnn_layer](x)
    ...
    def __init__(
        ...
        if self['tie_gnn_layers']:
            ...
            for layer_idx in range(self['n_gnn_layers']):
                ...
                if self['normalization'] == "batch":
                    norm_list.append(layers.BatchNormalization())
                elif self['normalization'] == "layer":
                    norm_list.append(layers.LayerNormalization(epsilon=1e-6))

            # store lists in instance variables which are not saved
            self._gnn_gcn = NoDependency(gcn_list)
            self._gnn_fully_connected = NoDependency(dense_list)

            if self['normalization'] in ["batch", "layer"]:
                self._gnn_norm = NoDependency(norm_list)
            else:
                self._gnn_norm = None
